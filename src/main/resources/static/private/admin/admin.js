...
app.controller("DashboardController", function ($scope, $http, $timeout, $window) {
  ...
  $scope.redirectToPage = function (key) {
    // Define your routing logic based on the key
    switch (key) {
      case 'Project':
        $window.location.href = 'report?project'; // Replace with your actual URL
        break;
      case 'Warranty':
        $window.location.href = 'report?Warranty'; // Replace with your actual URL
        break;
      case 'Insurance':
        $window.location.href = 'report?Insurance'; // Replace with your actual URL
        break;
      case 'AMC':
        $window.location.href = 'report?Amc'; // Replace with your actual URL
        break;
      case 'Log':
        $window.location.href = 'report?Log'; // Replace with your actual URL
        break;
      case 'Rent':
        $window.location.href = 'report'; // Replace with your actual URL
        break;
      default:
        console.warn('No page defined for key: ' + key);
        break;
    }
  };
...
app.controller("ReportController", function ($scope, $http, $timeout, DTOptionsBuilder, $q, $window) {
  $scope.form = {};
  $scope.views = {};
  $scope.views.list = true;
  $scope.assetTypeData = [];
  ...
  // Function to calculate days difference between end date and today
  $scope.getDaysDifference = function (endDate) {
    const today = new Date();
    const end = new Date(endDate);

    // Calculate the difference in time
    const diffTime = end - today; // in milliseconds
  ...
  // Function to check if the end date is near
  $scope.isEndDateNear = function (endDate) {
    const diffDays = $scope.getDaysDifference(endDate);

    // Return true if the difference is 10 days or less
  ...
  $scope.findAssetsWithLogLast7Days = function () {
    showHideLoad();
    $http({
      method: "GET",
      url: "report/findAssetsWithLogLast7Days",
    }).then(
      function successCallback(response) {
        // //console.log(response);
        $scope.findAssetsWithLog = response.data;
        showHideLoad(true);
      },
      function errorCallback(response) {
        //console.log(response.statusText);
    ...
    if (query) {
      switch (query) {
        case 'project':
          ...
          $scope.fetchProject(); // Call the function for project
          break;
        case 'Warranty':
          $scope.activeTab = query;
          $scope.Warranty(); // Call the warranty function
          break;
        case 'Amc':
          $scope.activeTab = query;
          $scope.Amc(); // Call the AMC function
          break;
        case 'Insurance':
          $scope.activeTab = query;
          $scope.Insurance(); // Call the insurance function
          break;
        case 'Log':
          $scope.activeTab = query;
          $scope.findAssetsWithLogLast7Days(); // Call the insurance function
          break;
        default:
          console.warn('No API call defined for hash: ' + query);
          break;
      }
    }
  }
