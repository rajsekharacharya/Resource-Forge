...
app.controller("AssetTypeController", function ($scope, $http, $timeout, DTOptionsBuilder) {
    ...
    $scope.isTypeDisabled = function(type) {
        return type === 'LAPTOP' || type === 'DESKTOP' || type === 'ALL-IN-ONE';
    };
...
app.controller("AssetController", function ($scope, $http, $timeout, DTOptionsBuilder, $q) {
    ...
    $scope.fetchAutoAssetDetails = function (data) {
        showHideLoad();
        $http({
            method: "GET",
            params: { data: data },
            ...
            function successCallback(response) {
                // console.log(response.data);
                // console.log($scope.form);
                $scope.autoAssetData = response.data;
                $scope.form.type = response.data.assetType;
                $scope.typeFinder(response.data.assetType);
                $scope.form.data01 = response.data.processor;
                $scope.form.data02 = response.data.availableRam;
                $scope.form.data03 = response.data.ssdTotal;
                $scope.form.data04 = response.data.hddTotal;
                $scope.form.data05 = response.data.ram;
                $scope.form.data06 = response.data.graphicsCard;
                $scope.form.data07 = response.data.graphicsCardVRam;
                $scope.form.data08 = response.data.os;
                $scope.form.data09 = response.data.antiVirus;
                $scope.form.data10 = response.data.macAddress;
                $scope.form.data11 = response.data.uuid;
                $scope.form.data12 = response.data.ipAddress;
                $scope.form.model = response.data.model;
                // $scope.form.manufacturer = response.data.manufacturer;
                $scope.form.serialNos = $scope.form.serialNos || [];
                $scope.form.serialNos.push(response.data.serialNumber);
                $scope.form.serialNo = response.data.serialNumber;
                showHideLoad(true);
            },
            function errorCallback(response) {
                //console.log(response.statusText);
